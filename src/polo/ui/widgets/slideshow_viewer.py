...
class Slide():
    ...
    :param prev_slide: Sldie. Previous slide in the slideshow to be displayed.
    ...
    def controls(self, next_slide=False, prev_slide=False):
        ...
        if self.current_slide:
            if next_slide:
                ...
                logger.info('Requested next slide in {}'.format(self))
            elif prev_slide:
                self.current_slide = self.current_slide.prev_slide
                logger.info('Requested previous slide in {}'.format(self))
...
class SlideshowViewer(PhotoViewer):
    ...
    def update_slides_from_filters(self, image_types, human, marco):
        '''
        ...
        if self.run:
            ...
            # BUG When images are passed with no marco classification
            # then no image are returned. This is an issue for
            # loading runs that do not need to be classified
            # ie runs that are not in the visible spectrum
            # might need some kind of marker that will be on that
            # so always returns images or add a place holder type classificaion
    ...
    def display_current_image(self):
        ...
        if isinstance(cur_img, Image):
            ...
            logger.info('Set pixelmap to {} from {}'.format(
