...
from polo.ui.designer.UI_spectrum_dialog import Ui_Dialog
    ...
    def link_current_selection(self):
        ...
        if self.validate_selection(selected_runs):
            for i in range(len(selected_runs)-1):
                ...
                #run_a.alt_spectrum = run_b
                run_a.link_to_alt_spectrum(run_b)
            first_run, last_run = self.loaded_runs[selected_runs[0]
                                                   ], self.loaded_runs[selected_runs[-1]]
            first_run.alt_spectrum = last_run.run_name
            last_run.alt_spectrum = first_run.run_name
