...
class ImportCandidate():
    ...
    def unrar(self):
        if self.is_rar:
            unrar_result = unrar_archive(self.path, target_dir=self.path.parent)
            print(type(unrar_result))
    ...
    def set_cocktail_menu(self):
        if 'date' in self.data and isinstance(self.data['date'], datetime):
            self.data['cocktail_menu'] = tim.get_menu_by_date(self.data['date'], type_='s')
            # print(self.data['cocktail_menu'].type_, 'menu type\n\n')
            # parse the type from file name need metadata key
    
    # def read_xml_data(self):
    #     self.data.update(XmlReader().find_and_read_plate_data(str(self.path)))

    def __hash__(self):
        return hash(self.path)

            # try to tell what kind of run import this is doing to be
...
class RunImporterDialog(QtWidgets.QDialog):
    ...
    def __init__(self, current_run_names, parent=None):
        ...
        self.ui.pushButton_4.clicked.connect(self._import_rar_archives)
        self.ui.pushButton_5.clicked.connect(self._import_image_dir)
        self.ui.listWidget.currentItemChanged.connect(self._handle_candidate_change)
    ...
    def _import_rar_archives(self):
        file_paths = self._open_browser()
        if file_paths:
            import_candidates = [ImportCandidate(p) for p in file_paths]
            import_candidates, drop_outs = self._test_candidate_paths(import_candidates)
            
            # start unrar
            import_candidates, failed_unrars = self._unrar_candidate_paths(import_candidates)
            if failed_unrars:
                make_message_box(
                    message='\n'.join(['The following imports failed:\n'] + failed_unrars),
                    parent=self
                ).exec_()
            if import_candidates:
                [imp_can.assign_run_type() for imp_can in import_candidates]
                self._display_candidate_paths(import_candidates)
            
    def _import_image_dir(self):
        file_paths = self._open_browser(rar=False)
        if file_paths:
            import_candidates = [ImportCandidate(p) for p in file_paths]
            import_candidates, drop_outs = self._test_candidate_paths(import_candidates)
            if import_candidates:
    ...
    def _remove_run(self):
        self.import_candidates.pop(self.selected_import.formated_path)
    ...
    def _display_candidate_paths(self, import_candidates):
        if self.import_candidates:
            all_paths = set(import_candidates).union(set(self.import_candidates.values()))
        else:
            all_paths = set(import_candidates)

        self.import_candidates = {
                str(can_path.path): can_path for can_path in all_paths
            }
        
        self.ui.listWidget.clear()
        self.ui.listWidget.addItems(list(self.import_candidates.keys()))
    ...
    def _populate_fields(self, import_candidate):
        print(import_candidate.path, type(import_candidate))
    ...
    def _enable_hwi_import_tools(self):
        items = (self.ui.gridLayout.itemAt(i) for i in range(self.ui.gridLayout.count()))
        for i in items:
            widget = i.widget()
    ...
    def _display_cocktail_files(self, menu_type=None):
        ...
        menus = [os.path.basename(menu.path) for menu in sorted(menus, key=lambda m: m.start_date)]
